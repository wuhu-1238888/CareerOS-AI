...
// (matching/coach/interview/resume-parse,见 mock-fixtures.ts),使 LLM_PROVIDER=mock 时浏览器端
// 也能走通全链路;未识别 Agent 保持回显行为(输入传递断言与旧测试依赖)。
...
export const defaultMockReply: MockReplyFn = (messages, options) => {
  ...
  if (options?.agentName === "resume-parse-agent") {
    ...
    return JSON.stringify(mockResumeParseFixture());
  }
