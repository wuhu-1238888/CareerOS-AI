"use client";
// AI 洞察卡(工作台 IA 重构):工作台四问之「AI 发现了什么」= 最近一次画像分析的摘要。
// 数据源:trpc.profile.get 的 aiAnalysis(仅画像分析,不混入岗位匹配报告;客户端渲染前校验,
// safeParse 先例 profile-result.tsx:119)。top-3 策略:优势/短板/建议各取前 3 条(优势先例
// profile-result.tsx:145;短板派生先例 :133-137,数据来自推荐方向的 weaknesses)。
// 降级纪律:未分析 / 加载失败 / 数据异常 → 卡内引导文案,绝不伪造 AI 结论;与 /profile 页
// 自身的异常提示(profile-result.tsx:121-128)保持一致。分析导向:只呈现 AI 原始文本,
// 不添加「建议你去完成 XX」的命令式措辞(那是「下一步建议」行动卡的职责)。
...
export function AiInsightCard({ analyzed }: { analyzed: boolean }) {
  ...
  const weaknessRows = analysis
    ? analysis.directions
        ...
        .slice(0, 3)
    : [];
  const suggestionRows = analysis ? analysis.suggestions.slice(0, 3) : [];
        ...
        <>
          <ul className="mt-4 space-y-4">
            ...
            {suggestionRows.length > 0 && (
              <li className="flex items-start gap-3">
                <p className="w-16 shrink-0 text-caption text-ink-faint">推荐行动</p>
                <ul className="min-w-0 space-y-2">
                  {suggestionRows.map((suggestion, index) => (
                    <li key={`${suggestion.gap}-${index}`} className="flex items-start gap-1.5">
                      <ArrowRight className="mt-0.5 size-4 shrink-0 text-green-600" aria-hidden />
                      <span className="min-w-0">
                        <span className="block text-caption text-ink-faint">{suggestion.gap}</span>
                        <span className="block text-body-sm text-ink">{suggestion.action}</span>
                      </span>
                    </li>
                  ))}
                </ul>
