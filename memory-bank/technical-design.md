...
**上传视图退出体验(4.14 修订)**:4.13 后上传视图仍是死胡同(误触/反悔/失败无退出路径、选文件即上传无确认、解析中不可中止)。4.14 补齐:① hub 新增 `uploadFrom: "resume" | "resumes"` state —— 简历中心「新增简历」改为 `/resume?upload=1&from=resumes`,`?upload=1` effect 按 from 显式赋值(其余入口 `enterUploadMode` 置 resume);effect 依赖用**值捕获**(`uploadFromParam = searchParams.get("from")`),避免 searchParams 对象身份变化反复触发。② `handleExitUpload` 三分支:`uploadFrom === "resumes"` 或**无可返回结果视图**(`!resume.data`,或 `extractError && !hasParsed`)→ `router.replace("/resumes")`;否则 `setUploadMode(false)` 回原视图(URL 已是 /resume 或 /resume?resumeId=)。面包屑父级标签 = 退出目标(简历中心 / 简历优化),父级点击与「← 返回」走同一 onExit。③ 上传三态(渲染分支顺序 uploading > selectedFile > 拖拽区,堵二次提交窗口):选文件/拖入只 `setSelectedFile` 入待确认态(校验保留在选择时机,非法文件即时 Banner);「开始分析」才发 fetch;解析中「取消上传」→ `AbortController.abort()`,catch 判 `controller.signal.aborted` **静默回已选文件态**(文件保留可重试,不显示错误);成功路径 response ok 即清 selectedFile(解析中卡不闪回);卸载 cleanup abort(对已 settle 请求 no-op)。竞态限制(注释说明):abort 时服务端可能已建行(孤儿行留在简历中心,可接受);客户端不建任何占位行 —— 行仅在上传完成后由服务端创建,「取消上传不创建空 Resume」由数据层保证。④ 关联修复:行切换 effect **首帧守卫**(`prevIdRef`,`undefined → id` 属加载不属切换,只记录不复位),修复冷加载 `/resume?upload=1` 被秒关的既有 bug。⑤ resume-center「新增简历」href 改为带 from=resumes。
...
**AI 分析进度卡死(4.17 修订)**:用户验收发现解析/优化流程「进度卡 60%(llm 事件后)、手动刷新才出结果」。架构事实:无队列/SSE/worker —— `resume.rewrite` mutation 在 HTTP 请求内 await 整条管线(orchestrator.ts:53 建 run → 阻塞 LLM → pipeline.ts:135 版本事务),进度事件随执行落 `AgentRun.progress`,客户端唯一实时通道 = 700ms 轮询 `resume.latestRun`;60% 平台期是「llm 事件在阻塞调用前落库、无心跳」的设计内现象,不是 bug 本身。**真因(情况 B+D 组合)**:完成判定 `rewriteDone = rewriteSucceeded && hasVersion` 依赖轮询缓存,而 mutation 结算瞬间(hasVersion=true + rewriteSubmitted=false)`enabled: !hasVersion || rewriteSubmitted` 把 latestRewrite **提前禁用**(禁用在服务端翻转后几十 ms 内发生,远早于下一次 700ms tick)→ 缓存冻结在 running、`resume.latestRun.invalidate()` 全仓库 0 命中、无任何补救通道 → 视图钉死「分析中」;刷新重新 fetch 到 succeeded + 版本即恢复。同款 bug 曾在 profile 流程以权威守卫修复(profile-hub.tsx:68 `recovering` 加 `!hasResult`),resume 的 4.9 修复(847f651)未对称补上。**4.17 修复(纯前端,后端零改动)**:① 两个 latestRun 查询 `enabled` 常开(`!resume.isLoading && !!resume.data`),refetchInterval 谓词不变(仅 running/在途时 700ms);② 完成判定加权威「版本-运行对应」:`rewriteDone = hasVersion && (!run || succeeded || version.createdAt > run.createdAt)` —— run 建于管线起点、版本建于结束事务,「版本比缓存 run 新」即证明该管线已完成,不依赖冻结的轮询缓存终态(序列化后 createdAt 为 ISO 字符串,必须 `new Date(...).getTime()` 比较);③ mutation 结算 finally 后 `invalidate latestRun({intent})` 立即拉终态;④ 改写恢复 effect 去掉 `!hasVersion`(重新优化 + 响应丢失时旧版本存在也必须 invalidate 拉新版本)、两个恢复 effect 成功时清会话错误(响应丢失但服务端完成不再钉死错误视图);⑤ analysis-view 加慢分析提示(running 超 60s 显示安抚文案,10s ticker 驱动、卸载清理)。未改:后端管线/心跳(卡点不在进度粒度在完成传播)、profile/navigator 同款 enabled 早停(其 recovering 已带权威守卫,无可见卡死,留后续)。
