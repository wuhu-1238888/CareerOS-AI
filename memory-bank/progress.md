...
- **最近更新**:2026-08-21,成长路线页 UI/UX 布局优化(宽内容区 + Timeline + 概览带 + 手风琴折叠)完成并推送
...
纯 UI/UX/Layout 优化:**业务逻辑 / 数据逻辑 / 任务逻辑 / AI 生成逻辑零改动**(仅 RoadmapTimeline 视图层重排 + hub 骨架宽度 1 行 + 页面描述文案 1 行)。两个交互决策经用户确认:阶段折叠 = 手风琴式(一次展开一个);顶部概览 = 单条 sticky 概览带。
...
- **阶段展开/折叠优化**:Set 多开 → 手风琴单开(`openStageId: string | null`),默认仍展开首个未完成阶段;点击已开阶段收起,点击其他阶段自动切换
...
- **测试结果**:新增 4 个用例(手风琴切换/概览带两区/全部完成不显示当前阶段/阶段序号与任务计数),全套 250/250(34 文件)+ typecheck/lint/build 零错误;grep 自检零硬编码色值零渐变;dev server 登录态 5 页面 200、未登录 307 中间件保护正常;临时检查用户与脚本已清理
...
- **成长路线 UI/UX 优化走查**:内容区扩大与左右留白、Timeline 清晰度、手风琴展开/收起、概览带吸顶与两区信息、任务行扫描性、Desktop/Tablet/Mobile 三档布局、无横向滚动、无 Console 报错
