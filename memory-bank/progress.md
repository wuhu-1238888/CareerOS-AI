...
- **最近更新**:2026-09-04,模拟面试综合报告评分展示优化:均分标签「内容均分/表达均分」→「内容能力/表达能力」并显式「/ 10」明示 10 分制;「基于 N 道已评估题计算」→ 动态「已评估 X / N 题」注记(分母取 interview.get.questionCount,不写死):部分=「当前评分仅供阶段性参考」、全部=「本次面试评分已完成」、0 道=「本场暂无评分」且不渲染分数;补辅助文案「评分标准:10 分制 · 基于已完成题目的回答进行评估」;917/917 测试全绿、typecheck/lint 零错误、build 成功;评分算法与数据零改动
...
浏览器人工验收:完成 1 道即结束 → 报告页显示「已评估 1 / 5 题 · 当前评分仅供阶段性参考」;全部完成后显示「已评估 5 / 5 题 · 本次面试评分已完成」;「/ 10」与标签在深色模式下可读;刷新后注记与分数一致。
