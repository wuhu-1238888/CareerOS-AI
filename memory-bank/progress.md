...
- **最近更新**:2026-09-06,一级页面「页面标题区垂直间距」统一(H1→副标题 4px 不变;副标题→下一块 56/32px → 40/48/24px;3 处类名改动);测试基线 935/97
...
# AI 洞察间距与 CTA 层级微调(2026-09-06)

## 背景

「发现 → 行动」重构后的最后一次视觉微调:压缩卡内纵向留白、拉开两个 CTA 主次、控制首屏高度。严格限定:只改 ai-insight-card.tsx 内容态间距类名,信息架构/文案/数据逻辑零改动。

## 调整明细(全部沿用 4px 网格 token,零新值)

- 副标题 → 岗位优势:`mt-4`(16px)→ `mt-3`(12px)
- 岗位优势 → 需要关注:`space-y-4`(16px)→ `space-y-3`(12px)
- 需要关注 → 分割线:footer `mt-4`(16px)→ `mt-3`(12px)
- 主按钮 ↔ ghost:`gap-2`(8px)→ `gap-x-5 gap-y-2`(横向 20px,窄屏换行纵向 8px)
- 已足够合理未改:标题-副标题 4px;列表内部行距 6px(14px 文字再压伤可读性);分割线-CTA 12px;卡片 p-6 24px 全站 card token

## 验证

- 定向(ai-insight-card + dashboard-view)12/12 + 15/15;全量 935/935(97 文件);typecheck / lint 零错误
- Playwright 前后对照走查(msedge 有头,用户 3000 dev server,同夹具 before/after):卡片高度 312.5px → 300.5px(压缩 12px);主按钮 ↔ ghost 横向间距 8px → 20px(实测 20.0px);主按钮 32px 高不变、两 CTA 同基线不重叠;1160px 桌面与 375px 窄屏均无横向溢出(实测 0px)、CTA 单行不换行;无页面错误;夹具级联清理残留 0

## 下一步

浏览器人工验收:紧凑节奏观感、CTA 主次间距、深色模式可读性。

# 一级页面「页面标题区垂直间距」统一(2026-09-06)

## 背景

用户验收意见:多个一级页面的页面标题区(标题 + 副标题)与下方内容之间留白过多,模拟面试、成长路线两页最明显(标题区下方是窄的 sticky 条,空白感被放大);要求统一节奏:H1 ↓ 8–12px 副标题 ↓ 40–48px Tab/操作区/第一块内容;H1→副标题「不要增加这一段间距,如果当前已经合理,不需要修改」(现状 4px 已合理,保持不变);不要超过 56px、不要压缩到 24px 以下。分页处理:面试/成长路线重点缩小,职业画像适当缩小,简历优化只统一节奏不过度压缩,岗位匹配保留卡片前呼吸感只做必要统一,工作台整体合理不压缩。实现方式:优先通过统一 PageHeader 组件,避免页面级魔法数字。

## 调整明细(3 处类名,全部沿用 4px 网格 token,零新值)

- `page-header.tsx`(统一组件,7 个使用点一次生效):`mb-8`(32px)→ `mb-6`(24px)
- `interview-chat.tsx`(对话态根):`py-6` → `pt-4 pb-6`(顶部 24→16px,底部 24px 不变)
- `roadmap-timeline.tsx`(时间线态根):`py-6` → `pt-4 pb-6`

节奏表(副标题 → 下一块,走查实测):模拟面试对话态 56→40 / 其余态 56→48;成长路线时间线态 56→40 / 其余态 56→48;职业画像 56→48;岗位匹配 56→48;简历 Tab 条 32→24;工作台 24 不变;设置/成长报告页(共享组件连带)32→24。H1→副标题全部保持 4px。

## 验证

- 定向 + 全量 935/935(97 文件);typecheck / lint 零错误
- Playwright 前后对照走查(msedge 有头,用户 3000 dev server,同夹具 before/after,逐页实测):6 页「副标题底 → 下一块顶」dashboard 24→24、profile 56→48、navigator 56→40、matching 56→48、resume 32→24、interview(对话态,夹具:进行中面试场次)56→40,全部与目标值一致(±0px);H1→副标题 6 页恒为 4.0px;375px 窄屏 5 页零横向溢出;无页面 JS 错误;夹具级联清理残留 0。发现(既有,非本次引入):职业画像 Hero 动作按钮行 375px 溢出 242px(按钮 whitespace-nowrap 不换行),留待人工确认
- 走查中一度 /api/auth/providers 500(`__webpack_modules__[moduleId] is not a function`,双 dev server 混写 .next 后遗症复发):改 auth route 入口文件触发深度重编译后 200,git checkout 还原后保持 200;另发现 3001/3002 存在遗留 dev server(未动,权限边界外)

## 下一步

浏览器人工验收:6 页标题区节奏观感(重点面试对话态与成长路线时间线态)、职业画像 Hero 按钮行 375px 溢出的处理决策。
