...
| AI Insight Card(分析摘要卡) | `card` 底 + 24px 内边距;标题 text-h2 + `ai-badge` + 副标题 13px `ink-muted`;三行 = 眉标 12px `ink-faint`(岗位优势/当前短板/重点关注)+ 条目 13px(优势 ✓ 绿 top-3 仅标题 / 短板 中性图标无红、AI 原文逐字不带来源前缀 top-2 / 重点关注 = 画像摘要结语句一行 准星图标 13px medium,摘要无有效句兜底首个建议 gap);空行不渲染;底部 ghost「查看完整分析」;不渲染建议 action;未分析/解析失败 → 卡内引导(不造假);四态齐全 |
