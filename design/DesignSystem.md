...
- 400 正文 / 500 可交互强调(按钮、选中项)/ 600 标题 / 700 仅大数字、引导页与 Landing 标题(`display-lg` Hero H1、`display` 区块 H2,见 DesignRules 首页章)。
- 13px 以下永远不用 700。
- 「700」的三个例外场合均为「非产品界面」:引导页、Landing 首页、大数字;内部业务页面一律 600 及以下。
...
- 页面头节奏(2026-09 一级页面统一,实现见 `PageHeader mb-6`):H1 与描述间距 4px(`space-y-1`);
  页面头与内容间距 24px(space-6);单视图顶部自带 16/24px 内边距(如 `pt-4 pb-6`)时合计约 40–48px,
  不叠加成 56px 及以上的空档。面试对话/路线时间线等「页面头 → sticky 条」场景取 40px 档,其余视图取 48px 档。
...
- 顶部工具条(仅管理/分析操作):版本选择器(常显,下拉 = 版本列表 + 分隔 + 版本管理区:＋ 另存为新版本 / 删除当前版本(危险色,仅剩一个时禁用))/ 重新分析 / 修改信息 / 上传新简历 / 查看全部简历。
- AI 建议区标题行:`已采纳 X/N` + 「全部接受」(secondary,点击先弹确认框,确认后一次性应用)。
- 最终文本预览区右上:[复制最终文本] secondary + [导出 PDF] primary(打开应用内 PDF 预览浮层)。
